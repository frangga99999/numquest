...
export default function NumberPad({ value, onChange, disabled = false, decimal = false, maxLength = 12 }) {
  ...
  return <div className="number-pad" role="group" aria-label="Keyboard angka">
    ...
    <div className="number-pad-play" aria-hidden="true"><span key={tap?.id} className={tap ? 'number-pad-friend hopping' : 'number-pad-friend'}><span>•ᴗ•</span></span>{tap && <span key={'spark'+tap.id} className="number-pad-spark">✦</span>}</div>
  </div>
}
