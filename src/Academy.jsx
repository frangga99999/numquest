...
export default function Academy({ g, setG, onClose }) {
  ...
  useEffect(()=>{if(!active||!timed||guide)return; const timer=setInterval(()=>setSeconds(s=>Math.max(0,s-1)),1000);return()=>clearInterval(timer)},[active,timed,guide])
 ...
 const progress=g.academy || {}
  ...
  return <main className="academy screen pr-system">
    ...
    {!selected ? <>
      ...
      <button className="btn ghost" onClick={()=>setGuide(true)}>Lihat panduan</button>
      {guide && <GuideDialog onClose={()=>setGuide(false)}><section className="ac-study"><span className="ac-eyebrow">{selected.level}</span><h2>{selected.title}</h2><p>{selected.lesson}</p><p className="ac-challenge-type">{selected.challenge}</p><div className="ac-example"><small>CONTOH TERURAI</small><p>{selected.example}</p></div></section></GuideDialog>}
